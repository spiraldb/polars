...
- **Vortex sink producer/writer error-path determinism** (`crates/polars-stream/src/nodes/io_sinks/writers/vortex/mod.rs:138-143`): MF-002's fix forwards producer errors through the chunk channel AND returns Err — two paths race on which error message surfaces first. Both paths produce a non-OK error (no silent data corruption), but the wording is scheduler-dependent (caller sees either the original `PolarsError` from `producer.await?` OR the `VortexError`-wrapped form from the writer). Polish: switch the join site to a `select!`-based pattern that explicitly aborts the writer on producer error, making the error-path message deterministic. Modest effort; out of scope for PR-1.3 (a 6-fix patch). (Deferred from PR-1.3 inner-loop cycle 1 should-fix #5, 2026-05-15.)
