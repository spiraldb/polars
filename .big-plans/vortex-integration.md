...
- **PR #3 (Phase 2)** on `vortex-integration` — pure perf follow-on stacked on this branch: AExpr-direct convertor walking `Arena<AExpr>` at IR-build time, replacing the legacy `SpecializedColumnPredicate` pushdown path. See that branch's plan for its sub-PR detail. (Originally PR #1; closed and reopened 2026-05-19 as PR #3 to match phase ordering — lower-numbered PR is the foundation.)
