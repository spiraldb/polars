...
last_user_touchpoint_what: "completed Phase 1.2 parallel exploration; entering design-tree interview"
...
- **No release to crates.io** in this branch. The workspace `vortex = { path = "../../../../vortex/vortex" }` blocks crates.io release; resolving that is a separate concern. Phase 1's CI green-up addresses CI feasibility, not crates.io publication.
...
| PR-13 architecture (Option A vs B vs hybrid) | DEFERRED | Subagent 7 recommends Option B (parallel paths) → migrate to Option A via PR-13.6 cutover. Trigger condition: user response to upcoming AskUserQuestion in Step 1.4. |
| CI green-up approach (Vortex path-dep) | DEFERRED | Four candidates: (a) Vortex crates.io publish, (b) git-rev dep, (c) CI workflow clone, (d) feature-gate vortex out of default CI. Trigger condition: user response to upcoming AskUserQuestion. |
| Per-phase review-counts | DEFERRED (sketched in handoff plan: 4/3/3/4) | Trigger condition: Step 1.6 AskUserQuestion. |
| PR-8 leads Parquet on `table_statistics` | DEFERRED | No Polars file format currently populates this from its own footer (only `PythonDataset`). Decide whether to lead the pattern or wait for upstream. Trigger condition: Step 1.4 AskUserQuestion. |
...
- **No regressing the path-dep release blocker.** New direct dependencies on `vortex`-internal symbols (vs the re-export through `polars_vortex::vortex`) deepen the coupling.
...
|---|---|---|---|---|---|
| 1 | Ratify + CI green | Retroactive 4-vote gauntlet of cumulative diff vs `main` + CI green-up (Vortex path-dep) + cheap polish | (a) 4-vote phase-end review accepts. (b) `cargo check -p polars --features vortex,cloud,parquet,dtype-full` clean. (c) `cargo test -p polars-vortex --features dtype-date,dtype-datetime,dtype-time,dtype-decimal` → 65 Rust tests pass. (d) `pytest py-polars/tests/unit/io/test_vortex.py` → 8 tests pass. (e) `gh pr checks 1 --repo spiraldb/polars` shows green for Rust + Python core checks. | 3-4 | 4-vote |
| 2 | PR-13 aggressive AExpr pushdown | New convertor module (Option B trajectory per Subagent 7); arithmetic / CAST / struct field access / temporal extracts shipped incrementally; final commit deletes the `SpecializedColumnPredicate` fast path | (a) 3-vote review accepts. (b) Every row in existing plan's §5 pushdown coverage table implemented + tested OR documented as deliberately deferred. (c) New e2e tests verify pushdown engagement via Vortex `Expression::display_tree()` or `POLARS_VERBOSE` log assertion. (d) Build/test suite still green. | 5-6 | 3-vote |
| 3 | PR-8 file-stats + PR-6 multi-file / nested coverage | Populate `UnifiedScanArgs::table_statistics` from Vortex footer; add multi-file scan tests + schema-evolution policy round-trips (`missing_columns`/`extra_columns`/`cast_options`) + nested-type (List/Struct) end-to-end + small-int dtypes (i8/i16/u8/u16) | (a) 3-vote review accepts. (b) Multi-file scan with file-level stats shows whole-file skips (via `EXPLAIN` or Vortex pruning counters). (c) Schema-evolution policy tests pass across the missing/extra/cast matrix. (d) Nested-type roundtrip tests pass. (e) Small-int dtype roundtrip tests pass. (f) Build/test suite green. | 3-4 | 3-vote |
...
|---|---|---|---|---|
| PR-1.1 | 1 | CI green-up: resolve Vortex path-dep blocker | `Cargo.toml` (workspace), possibly `.github/workflows/{test-rust,test-python,lint-rust}.yml` | `gh pr checks 1` shows green on clippy-stable / check-features / test (ubuntu) / Lint Rust / build-rust-docs |
...
| PR-3.1 | 3 | PR-8 — File-level stats → `UnifiedScanArgs::table_statistics` from Vortex footer | `crates/polars-plan/src/plans/conversion/dsl_to_ir/scans.rs:293-359`, `crates/polars-vortex/src/read/file_stats.rs` (new) | Multi-file scan with stats-pruning shows whole-file skips via `EXPLAIN` or pruning-counter inspection; new module unit-tested |
...
1. **Vortex path-dep blocks CI** — P=high; impact=severe. All CI checks fail because `cargo metadata` can't find `/home/vortex/vortex/Cargo.toml` on GHA runners. Mitigation: PR-1.1 resolves before any other Phase 1 work. Explicit decision in Step 1.4 design-tree interview.
...
4. **PR-8 leads Parquet on `UnifiedScanArgs::table_statistics`** — P=low (technical); impact=moderate (upstream-adoption-risk). No Polars file format currently populates this from its own footer (only `PythonDataset` via Iceberg/Delta). Going first introduces an API pattern that upstream may want to shape differently. Mitigation: surface to user in Step 1.4 ("lead the pattern or wait?"). If "lead": design the `VortexFile::file_stats() → DataFrame` shape carefully in PR-3.1; consider proposing the pattern upstream before merge.
...
- **Workspace `vortex = { path = "../../../../vortex/vortex" }` → crates.io version**: blocks public crates.io release. Deferred separately; Phase 1's CI green-up (PR-1.1) addresses CI feasibility, not crates.io publication.
...
- **Workspace `vortex = { path = "../../../../vortex/vortex" }` path-dep**: accepted as a release blocker for now. Phase 1's CI green-up resolves CI feasibility without resolving the crates.io release blocker; the latter is a separate concern.
