...
polars-vortex crate                                                        (2,697 LoC, 43 unit + 23 integration = 66 Rust tests)
...
- **PR #4 (Phase 2)** on `vortex-integration-phase-2` — pure perf follow-on stacked on this branch: AExpr-direct convertor walking `Arena<AExpr>` at IR-build time, replacing the legacy `SpecializedColumnPredicate` pushdown path. See that branch's plan for its sub-PR detail. (Two aesthetic moves 2026-05-19: original PR #1 closed → reopened as PR #3 to put foundation at the lower number; then PR #3 closed → reopened as PR #4 with the branch renamed from `vortex-integration` to `vortex-integration-phase-2` so the branch-name pair `phase-1` + `phase-2` is symmetric.)

This branch's plan describes Phase 1++ (the foundation + Phase 3 absorption + remaining work). Phase 2's plan (in branch `vortex-integration-phase-2`) describes the convertor work in detail.
...
After PR-3.3 + PR-4.x land, this branch hits its phase-end 4-vote gauntlet. Then Phase 2 (`vortex-integration-phase-2`) rebases onto the new tip and the project is ready to ship as the 2-PR stack.
...
Total: ~15 PRs across 4 phases (added PR-2.0 in cycle-4 re-plan). Each PR fits the 1-3 commit granularity per `/big-plans` discipline.
